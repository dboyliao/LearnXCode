...
=begin
	I want a single CSV file that lists:
...
$season_opens = Date.strptime '2014-03-24'
HEADERS = %w{ firstName lastName attempts completions interceptions
              touchdowns yards whenPlayed ourTeam ourScore
		          theirTeam theirScore }
SEASONS = 8
csv = CSV.new($stdout, row_sep: "\r\n")
# RFC 4140 (http://tools.ietf.org/html/rfc4180)
# says the "standard" line ending is CRLF
...
SEASONS.times do
	...
	Team.each_team do | team |
		...
		team.each_other_team do | opponent |
			whenPlayed = $season_opens + week

			qb_records = record_game(team, opponent, whenPlayed)
			for quarterback in qb_records
				csv << quarterback
			end
			week += 7
		end # Each other Team
	...
	# End of season. Set the next opening date and shuffle the players.
